...
const CAT_ICONS = {
  ...
  react: "⚛️", reactnative: "📱", nextjs: "▲",
};
const CAT_LABELS = {
  html: "HTML", css: "CSS", javascript: "JavaScript", typescript: "TypeScript",
  react: "React", reactnative: "React Native", nextjs: "Next.js",
};
...
export default function Progress() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white px-4 py-8 max-w-4xl mx-auto space-y-8">
            ...
            <>
              ...
              <div className="flex justify-center gap-2 mt-2">
                {Array.from({ length: 7 }).map((_, i) => {
                  ...
                  return (
                    <div key={i} className={`w-7 h-7 rounded-full flex items-center justify-center text-sm transition-all ${filled ? "bg-orange-500 text-white" : "bg-slate-800 text-slate-600"}`}>
                      {filled ? "🔥" : "○"}
                    </div>
                  );
                })}
          ...
          {loadingStreak || !challenge ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="text-xs px-2 py-1 bg-slate-800 rounded-full text-slate-400">{CAT_LABELS[challenge.category]}</span>
                <span className="text-xs text-slate-500">{challenge.topic} → {challenge.title}</span>
              </div>
              <p className="text-sm text-white font-medium">{challenge.question}</p>
              <AnimatePresence>
                {showAnswer && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                    className="text-xs text-slate-300 bg-slate-800 rounded-lg p-3 leading-relaxed">
                    {challenge.summary}
                  </motion.div>
                )}
              </AnimatePresence>
              <div className="flex gap-2">
                <button onClick={() => setShowAnswer((v) => !v)}
                  className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs font-semibold transition-all">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {loadingAreas ? (
            Array.from({ length: 7 }).map((_, i) => (
              <div key={i} className="h-24 animate-pulse bg-slate-900 rounded-xl" />
            ))
          ) : (
            areas.map((a) => (
