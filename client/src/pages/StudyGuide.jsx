  ...
  async function fetchSummary() {
    ...
    try {
      const { data } = await api.post("/study/summarise", {
        ...
        summary: topic.summary, explanation: topic.explanation, code: topic.code,
      });
      setSummary(data.summary);
    } catch { setSummary("AI summary unavailable. Check your connection or try again."); }
  ...
  async function sendQuestion() {
    ...
    try {
      const { data } = await api.post("/study/ask", {
        title: topic.title, topic: topic.topic,
        explanation: topic.explanation, code: topic.code, question: q,
      });
      setMessages(m => [...m, { role: "ai", text: data.answer }]);
    } catch {
      setMessages(m => [...m, { role: "ai", text: "AI unavailable right now. Try again." }]);
    }
    finally { setLoadAsk(false); }
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-indigo-200 dark:border-indigo-800 bg-indigo-100/60 dark:bg-indigo-900/40">
        ...
        <span className="font-semibold text-sm text-indigo-700 dark:text-indigo-300 flex-1">AI Assistant</span>
        {[["summary","✨ Summarise"],["ask","💬 Ask AI"]].map(([m,l]) => (
        ...
        <button onClick={onClose} className="text-slate-400 hover:text-slate-600 ml-1 text-xl leading-none">×</button>
      </div>
      ...
      {mode === "summary" && (
        <div className="p-4">
          {loadingSum ? (
            ...
            <>
              <p className="text-sm text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{summary}</p>
              <button onClick={fetchSummary} className="mt-3 text-xs text-indigo-500 hover:text-indigo-700 underline">↻ Regenerate</button>
            </>
          )}
        </div>
      )}
      ...
      {mode === "ask" && (
        <div className="flex flex-col">
          <div ref={chatRef} className="overflow-y-auto p-4 space-y-3" style={{ maxHeight: 240 }}>
            {messages.length === 0 && (
              <p className="text-xs text-slate-400 italic">Ask anything about <strong>{topic.title}</strong>…</p>
            )}
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role==="user"?"justify-end":"justify-start"}`}>
                <div className={`max-w-[88%] rounded-xl px-3 py-2 text-sm leading-relaxed ${
                  ...
                  {m.role==="ai" && <span className="text-[10px] text-indigo-500 font-bold block mb-0.5">🤖 AI</span>}
...
function TopicCard({ topic, reviewed, onToggle }) {
  ...
  return (
    <div style={{ cursor: "pointer" }} className={`rounded-2xl border transition-all ${reviewed?"border-indigo-400 dark:border-indigo-500":"border-slate-200 dark:border-slate-700"} bg-white dark:bg-slate-800/60 overflow-hidden hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-md`}>
      <button onClick={() => setOpen(v => !v)} style={{ cursor: "pointer" }} className="w-full text-left px-4 py-3 flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5 mb-1">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${DIFF_STYLES[topic.difficulty]}`}>{topic.difficulty}</span>
            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">{topic.topic}</span>
            {reviewed && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-300">✓ Reviewed</span>}
          </div>
          ...
          <p className="text-xs text-indigo-600 dark:text-indigo-300 font-medium mt-1 line-clamp-1">💡 {topic.summary}</p>
        </div>
          ...
          >
            <div className="px-4 pb-4 border-t border-slate-100 dark:border-slate-700 pt-3 space-y-3">
              ...
              <div className="bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800 rounded-xl p-3">
                <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-400 mb-1">Key Concept</p>
                <p className="text-sm font-medium text-indigo-700 dark:text-indigo-200 leading-relaxed">{topic.summary}</p>
              </div>

              {/* Tabs */}
              <div className="flex gap-1.5 flex-wrap">
                {tabs.map(t => (
                  <button key={t.id} onClick={() => setTab(t.id)} style={{ cursor: "pointer" }}
              ...
              {tab==="explanation" && (
                <div className="space-y-3">
                  ...
                  {topic.explanation.split(/\n\n+/).map((para, i) => (
                    <p key={i} className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{para.trim()}</p>
                  ))}
                  {/* If code exists, always show a preview */}
                  {topic.code && (
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Code Example</p>
              ...
              {tab==="question" && (
                <div className="space-y-3">
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-xl p-3">
                    <p className="text-xs font-bold text-amber-600 dark:text-amber-400 mb-1.5">🎯 Common Interview Question</p>
                    <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 leading-relaxed">{topic.interviewQuestion}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700 rounded-xl p-3">
                    <p className="text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-wide">💬 Model Answer</p>
                    {topic.explanation.split(/\n\n+/).map((para, i) => (
                      <p key={i} className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed mb-2 whitespace-pre-wrap">{para.trim()}</p>
                    ))}
                  </div>
                </div>
              )}
              ...
              <AnimatePresence>
                {showAi && <AiPanel key="ai" topic={topic} onClose={() => setShowAi(false)} />}
              </AnimatePresence>
...
function AiExplainer() {
  ...
  async function explain() {
    ...
    try {
      ...
      setHistory(h => [{ input: text, explanation: data.explanation }, ...h].slice(0, 10));
    } catch {
    ...
    <div className="rounded-2xl border border-violet-200 dark:border-violet-800 bg-gradient-to-br from-violet-50 to-indigo-50 dark:from-violet-950/40 dark:to-indigo-950/40 overflow-hidden">
      ...
      >
        ...
        <div className="flex-1">
          <p className="font-bold text-sm text-violet-700 dark:text-violet-300">AI Explainer</p>
          <p className="text-xs text-violet-500 dark:text-violet-400">Paste any code, concept or error — AI explains it</p>
        </div>
        ...
        {open && (
          ...
          >
            <div className="px-4 pb-4 space-y-3 border-t border-violet-200 dark:border-violet-800 pt-3">
              {/* Quick examples */}
              <div className="flex gap-1.5 flex-wrap">
                <span className="text-xs text-violet-400 mr-1 self-center">Try:</span>
                {EXAMPLES.map(ex => (
              ...
              <textarea
                ...
                placeholder={"Paste code, an error message, or any concept…\n\nExamples:\n• useCallback(() => ..., [deps])\n• Cannot read properties of undefined\n• What is the event loop?"}
                rows={7}
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {loading ? <><span className="animate-spin">⟳</span> Explaining…</> : <>✨ Explain This</>}
                </button>
              </div>
              ...
              <AnimatePresence>
                {loading && !result && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 text-sm text-violet-500 py-2">
                    <span className="animate-spin inline-block">⟳</span> AI is analysing…
                  </motion.div>
                )}
                {result && (
                  <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-bold text-violet-500 uppercase tracking-wide">🤖 AI Explanation</span>
                      <button onClick={() => { setInput(""); setResult(null); }}
                        className="text-xs text-slate-400 hover:text-slate-600">✕ Clear</button>
                    </div>
                    <p className="text-sm text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{result}</p>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {history.length > 1 && (
                <div className="space-y-1">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">Recent</p>
                  {history.slice(1, 4).map((h, i) => (
                    <button key={i} onClick={() => { setInput(h.input); setResult(h.explanation); }}
...
// ── Dev Tools Panel ──────────────────────────────────────────────────────────
const DEV_TOOLS = [
  { id: "ts",     icon: "🔷", label: "TS Adder",      desc: "Add TypeScript types to JS code",          endpoint: "/study/ts-add",       inputPlaceholder: "Paste JavaScript code here…",         btnLabel: "Add TypeScript →", resultLabel: "TypeScript Output" },
  { id: "errors", icon: "🐛", label: "Error Finder",  desc: "Find bugs and errors in any code",          endpoint: "/study/find-errors",  inputPlaceholder: "Paste any code snippet here…",        btnLabel: "Find Errors →",    resultLabel: "Errors Found" },
  { id: "breaks", icon: "💥", label: "Break Finder",  desc: "Find code that could break at runtime",     endpoint: "/study/find-breaks",  inputPlaceholder: "Paste code to check for risk areas…", btnLabel: "Find Break Risks →", resultLabel: "Risk Report" },
  { id: "js",     icon: "⚡", label: "JS Compiler",   desc: "Run JavaScript code instantly in-browser",  endpoint: null,                  inputPlaceholder: 'console.log("Hello World!");\n// Write any JavaScript…', btnLabel: "Run Code ▶", resultLabel: "Output" },
];
...
function DevTools() {
  ...
  function switchTool(id) { setTool(id); setCode(""); setResult(null); }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/60 overflow-hidden">
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
        ...
        <div className="flex-1">
          <p className="font-bold text-sm text-slate-800 dark:text-slate-100">Dev Tools</p>
          <p className="text-xs text-slate-500 dark:text-slate-400">TS Adder · Error Finder · Break Finder · JS Compiler</p>
        </div>
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-slate-100 dark:border-slate-700 pt-3">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">{cur.desc}</p>

              {/* Code input */}
              <textarea value={code} onChange={e => setCode(e.target.value)}
                onKeyDown={e => e.key === "Enter" && e.metaKey && run()}
                placeholder={cur.inputPlaceholder} rows={8}
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-slate-400">{isMac ? "⌘" : "Ctrl"} + Enter to run</span>
                  {cur.id === "js" && (
                    <button onClick={() => navigate("/js-compiler")}
                ...
                <button onClick={run} disabled={!code.trim() || loading}
                  ...
                  {loading ? <><span className="animate-spin inline-block">⟳</span> Processing…</> : cur.btnLabel}
                </button>
              </div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    ...
                    <div className="flex items-center justify-between px-4 py-2 bg-slate-800">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="ml-2 text-[10px] text-slate-400 font-mono">{cur.resultLabel}</span>
                      </div>
                      {cur.id === "ts" && result.ok && (
                    ...
                    <pre className={`text-xs p-4 overflow-x-auto whitespace-pre-wrap leading-relaxed font-mono bg-[#0d1117] ${result.ok ? "text-emerald-400" : "text-red-400"}`}>
...
export default function StudyGuide() {
  ...
  const filtered = useMemo(() => allInCat.filter(t => {
    ...
    return !q || t.title.toLowerCase().includes(q) || t.summary.toLowerCase().includes(q) || t.topic.toLowerCase().includes(q);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">275 topics · 8 technologies · AI-powered explanations & Q&A</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/60 p-4">
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{cat?.label} Progress</p>
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{reviewedCount} / {allInCat.length} reviewed</p>
          </div>
          <span className="text-2xl font-black text-indigo-500">{Math.round(progress*100)}%</span>
        </div>
        <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
          <motion.div className="h-2 rounded-full bg-indigo-500" animate={{ width:`${progress*100}%` }} transition={{ duration:0.4 }} />
        </div>
        {cat?.sources?.length > 0 && (
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">📚 Reliable Sources:</span>
            {cat.sources.map(s => (
      ...
      <p className="text-xs text-slate-400">
        Showing <strong className="text-slate-600 dark:text-slate-300">{filtered.length}</strong> of {allInCat.length} topics
        {activeDiff!=="All"&&` · ${activeDiff}`}{search&&` · "${search}"`}
      </p>
