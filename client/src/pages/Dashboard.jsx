              ...
              <div className="space-y-2">
                ...
                {i === 2 &&
                  stats.byType?.map((t) => (
                    ...
                    >
                      <span>
                        {t.type === "Coding" ? "💻 Coding" : "🧩 Technical"}
                      </span>
        ...
        <div className="space-y-3">
          {loading
            ? Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between gap-3 pb-3 border-b border-black/5 dark:border-white/5"
                >
                  <div className="flex-1 space-y-2">
            ...
            : stats.recent?.map((r) => (
                <Wrapper
                  key={r.id}
                  {...(!user?.isGuest && { to: `/question/${r.id}` })}
                  className={`flex items-center justify-between text-sm border-b border-black/5 dark:border-white/5 pb-2 last:border-0 rounded-lg px-2 -mx-2 ${
                    user?.isGuest
                      ? "cursor-default"
                      : "hover:bg-slate-50 dark:hover:bg-slate-800/50 transition cursor-pointer"
                  }`}
                >
                  <div>
