...
export default function Notifications() {
  ...
  const markOneRead = async (id) => {
    try {
      await api.patch(`/admin/notifications/my/${id}/read`);
      ...
      setItems((prev) => {
        ...
        updateCount(updated.filter((n) => !n.read).length);
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-16 animate-pulse bg-slate-100 dark:bg-slate-800 rounded-xl"
            />
          ))}
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-20 text-slate-400">
          ...
          <p className="text-sm mt-1">
            You'll see push notifications and study reminders here.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p
                      className={`text-sm font-semibold leading-tight ${n.read ? "text-slate-600 dark:text-slate-300" : "text-slate-800 dark:text-white"}`}
                    >
                      {n.title}
                    </p>
