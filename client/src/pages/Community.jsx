...
const MemoCard = memo(
  ({ q, onUpvote, onHighlight, onBookmark, onDelete, isAdmin }) => (
    <QuestionCard
      q={q}
      onUpvote={onUpvote}
...
export default function Community() {
  ...
  const updateQ = useCallback(
    (updated) =>
      setQuestions((qs) =>
        qs.map((x) => (x.id === updated.id ? { ...x, ...updated } : x)),
      ),
    [],
  );
  ...
  const toggleUpvote = useCallback(
    async (q) => {
      if (isGuest) {
        toast.error("Create an account to upvote questions", {
          icon: "🔒",
        });
        return;
      }
      ...
      try {
        ...
        toast.success(
          data.isUpvoted ? "Upvoted successfully" : "Upvote removed",
          {
            id: toastId,
          },
        );
  ...
  const toggleBookmark = useCallback(
    async (q) => {
      if (isGuest) {
        toast.error("Create an account to bookmark questions", {
          icon: "🔒",
        });
        return;
      }
      ...
      try {
        ...
        toast.error(err.response?.data?.detail || "Failed to bookmark", {
          id: toastId,
        });
      }
    },
          ...
          <div className="space-y-2">
            ...
            >
              {todayPoster.allowed ? (
                ...
                <>
                  <span>🚫</span>
                  <span>Community is closed today — no posts scheduled.</span>
                </>
              )}
            </div>
            {todayPoster.myDay &&
